    ...
    foreach ($comments as $comment) {
        if ($comment['parent_id'] == $parent_id) {
            ...
            <div class="comment">
                ...
                ' . show_write_comment_form($comment['id'], 'child') . '
                <div class="replies">
...
function show_write_comment_form($parent_id = -1, $scope = 'parent') {
    $display = $scope == 'parent' ? '' : 'style="display:none"';
    $comm_btn = $scope == 'parent' ? 'Submit Comment' : 'Submit Reply';
    $cncl_rpl = $scope == 'child' ? '<button type="button" onclick="cancelReply()" class="mt-2 btn btn-danger">Cancel Reply</button>' : '';
    $baseForm = $scope  == 'parent' ? 'id="baseForm"' : '';
    $html = '
    <div class="write_comment my-4" data-comment-id="' . $parent_id . '" '. $display .' '. $baseForm .'>
        <form>
            <input name="parent_id" type="hidden" value="' . $parent_id . '">
            <input name="name" type="text" class="form-control" placeholder="Your Name" required>
            <textarea name="content" class="form-control my-4" placeholder="Write your comment here..." required></textarea>
            <button class="btn mt-2 btn-success" type="submit">'. $comm_btn .'</button>
            '. $cncl_rpl .'
        </form>
    </div>
    ';
...
if (isset($_GET['post_id'])) {
    ...
    if (isset($_POST['name'], $_POST['content'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO comments (post_id, parent_id, name, content, submit_date) VALUES (?,?,?,?,NOW())');
        $stmt->execute([ $_GET['post_id'], $_POST['parent_id'], $_POST['name'], $_POST['content'] ]);

        exit('Your comment has been submitted!');

    }
    // Get all comments by the Page ID ordered by the submit date
    $stmt = $pdo->prepare('SELECT * FROM comments WHERE post_id = ? ORDER BY submit_date DESC');
